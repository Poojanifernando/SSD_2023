...
function EmpLeaveEdit() {
  ...
  function checkSpecialCharacters(input) {
    // Define a regular expression for allowed characters (capital and simple letters and numbers only)
  ...
  const ChangeOnClick = async (e) => {
    ...
    // Validate each input field for special characters
    if (
      !checkSpecialCharacters(Emp_Name) ||
      !checkSpecialCharacters(Emp_ID) ||
      !checkSpecialCharacters(Leave_Reason_Other) ||
      !checkSpecialCharacters(Comments)
    ) {
      alert("Special characters are not allowed in input fields.");
      return;
    }
    ...
    console.log(id);
    await axios
      .put(`http://localhost:5000/UpdateLeaveReq/${id?.id}`, EmpLeaveUpdate)
      .then((res) => {
  ...
  useEffect(function effectFunction() {
    ...
    axios
      .get(`http://localhost:5000/GetOneLeaveRequest/${id?.id}`)
      .then((res) => {
    ...
    <div>
      ...
      >
        <form>
          ...
          >
            *Default Type/Pending Results Are "undifined"{" "}
          </label>

          <table
            className="tableLeaveEdit"
            style={{ background: "rgba(80,80,80,0.45)" }}
          >
            <br />
            ...
            <tr>
              <td colspan="2">
                <div className="form-group">
                  ...
                  >
                    If Its "Other", Type The Reason Here:
                  </label>
                  <input
                    type="text"
                    className="form-control"
